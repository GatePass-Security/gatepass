...
const ROUTES = [
  {
    ...
    items: [
      ...
      { method: "GET", path: "/v1/scans/:id/findings", where: "Scan detail, Findings" },
    ...
    <div className="space-y-6">
      ...
      <Card header={<CardTitle icon={<ServerCog size={15} aria-hidden="true" />}>Deployment</CardTitle>}>
        <dl className="grid gap-x-8 gap-y-3 sm:grid-cols-2">
          <Row label="API base">
            <span className="font-mono text-[0.78rem] break-all">{API_BASE}</span>
          </Row>
...
async function probeWrite(path: string): Promise<Configured> {
  try {
    ...
    // Any non-401 means the request got past the credential guard.
    return "yes";
  } catch {
    return "unreachable";
  }
