...
const INTERPRETED_ROLE = new Set([
  // executed
  "cmd", "cmds", "command", "commands", "shell", "script", "scripts", "exec", "entrypoint",
  "argv", "arg", "args", "argument", "arguments", "flag", "flags", "option", "options",
  "code", "eval", "snippet", "expression", "expr", "formula", "predicate", "condition",
  // queried
  "sql", "query", "queries", "statement", "clause", "where", "filter", "filters", "selector",
  "xpath", "jsonpath", "jq", "cypher", "graphql", "aggregation", "pipeline",
  // matched as a program in its own right
  "pattern", "patterns", "glob", "regex", "regexp", "template", "format",
  // located
  "path", "paths", "file", "files", "filename", "filepath", "dir", "dirs", "directory",
  "directories", "folder", "cwd", "workdir", "destination", "dest", "target", "source", "src",
  "location", "prefix", "bucket", "table", "collection", "namespace",
  // addressed
  "url", "uri", "endpoint", "host", "hostname", "domain", "address", "origin", "callback",
  "webhook", "redirect", "proxy",
  // carried through verbatim
  "body", "content", "contents", "payload", "blob", "raw", "document", "html", "markup",
...
function csToolsOf(content: string): CsTool[] {
  ...
  while ((m = CS_TOOL_ATTR.exec(content)) !== null) {
    ...
    for (const raw of splitTopLevel(content.slice(paren + 1, end))) {
      ...
      const decl = /^(?:(?:this|params|ref|out|in)\s+)*([A-Za-z_][\w.]*(?:\s*<[\s\S]*>)?(?:\s*\[\s*\])?\s*\??)\s+([A-Za-z_]\w*)$/.exec(
        rest,
      );
      if (!decl) continue;
